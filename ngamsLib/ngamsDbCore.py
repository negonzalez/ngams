...
#******************************************************************************
#
# "@(#) $Id: ngamsDbCore.py,v 1.15 2012/05/25 11:23:38 amanning Exp $"
#
# Who       When        What
# --------  ----------  -------------------------------------------------------
...
class ngamsDbCore:
    ...
    def query(self,
        ...
        # changed from (not maxRetries) because I want to be able to pass in a value of 0, meaning don't retry the SQL
        if (maxRetries is None): maxRetries = self.__maxRetries
    ...
    def _query(self,
        ...
        startTime = time.time()
        try:
            res = self.__dbDrv.query(sqlQuery, parameters)
        except Exception, e:
            ...
            if (recurseLevel < maxRetries):
                ...
                recurseLevel += 1
                # instead of completely tearing down the old connection and rebuilding it by
                # calling self._connect(), we simply pass responsibility on to the actual DB
                # driver itself. This allows better support for the Oracle connection pool.
                # In that case we don't want to create a new connection pool, just retry
                # the query with a fresh connection from the pool
                notice("closing the old connection")
                self.__dbDrv.close()
                notice("rebuilding the connection")
                self.__dbDrv.connect(self.__dbServer, self.__dbName, self.__dbUser,
                              self.__dbPassword, self.__dbInterface,
                              recurseLevel)
                notice("Reconnected to DB server")
                notice("Retrying Query: %s" % sqlQuery)
