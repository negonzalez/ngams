...
#******************************************************************************
#
# "@(#) $Id: ngamsPClient.py,v 1.8 2010/10/26 11:32:35 mbauhofe Exp $"
#
# Who       When        What
# --------  ----------  -------------------------------------------------------
...
class ngamsPClient:
    ...
    def archive(self,
        ...
        if (ngamsLib.isArchivePull(fileUri)):
            ...
            res = self.sendCmd(NGAMS_ARCHIVE_CMD, wait, "", locPars)
        else:
            res = self.pushFile(fileUri, mimeType, wait, noVersioning, locPars)
...
#----------------------------------------------------------------------------------------
    ...
    def discartFile(self,
                diskId,
                fileId,
                fileVersion = -1,
                execute = 0):
        """
        Send an DISCART command to the NG/AMS Server associated to the object.
        ...
        Returns:      NG/AMS Status object (ngamsStatus).
        """
        return self.sendCmd(NGAMS_DISCARD_CMD, 1, "",
                            [["disk_id", diskId], ["file_id", fileId],
                             ["file_version", fileVersion],
                             ["execute", execute]])
    ...
    def fileStatus(self, fileId):                        
        """
        Send an Status command to the NG/AMS Server associated to the object.
        ...
        Returns:      NG/AMS Status object (ngamsStatus).
        """
        return self.sendCmd(NGAMS_STATUS_CMD, 1, "",
                            [["file_id", fileId]])
     
                       
                            
    def diskStatus(self, diskId):                        
        """
        Send an Status command to the NG/AMS Server associated to the object.
        ...
        try:
            ...
            reply, msg, hdrs, data =\
                   self._httpGet(host, port, cmd, wait, locPars, outputFile,
                                 None, self.getTimeOut(), returnFileObj, 
