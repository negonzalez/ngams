...
def genFileInfo(dbConObj,
    ...
    info(3, "Target name for file is: " + complFilename)
...
def ngamsGeneric(srvObj,reqPropsObj):
    ...
    info(3, "Mirroring plug-in handling data for file: %s" % os.path.basename(reqPropsObj.getFileUri()))
    ...
    if ((format.find("multipart") >= 0) or (format.find("multialma") >= 0)):
        info(3, "applying plug-in specific treatment")
    ...
    info(3, "File with URI %s is being handled by ngamsGeneric: format=%s file_id=%s file_version=%s finalName=%s" % \
    ...
    try:
        ...
        if not format: format = ngamsPlugInApi.determineMimeType(srvObj.getCfg(),stagingFilename)
