...
#******************************************************************************
#
# "@(#) $Id: ngamsTestLib.py,v 1.18 2010/10/04 19:13:26 jagonzal Exp $"
#
# Who       When        What
# --------  ----------  -------------------------------------------------------
...
# QUEUE=15m
###########################################################################
fo = open("/etc/sysconfig/sendmail")
lines = fo.readlines()
fo.close()
foundDaemonYes = 0
for line in lines:
...
if (not foundDaemonYes):
    raise Exception, "Mail configuration incorrect. Set parameter: " +\
          "DAEMON=yes in /etc/sysconfig/sendmail"
...
def delNgasTbls(dbObj):
    ...
    #jagonzal: The tests should run vs totally clean
    #          test db for consistency reasons
    ...
    dbObj.query("DELETE FROM ngas_hosts")
...
def isFloat(val):
	...
	#jagonzal: If it exceeds char (8 bits) range we also consider it as float
	if (val > 255): return 1
	else: return 0
...
class ngamsTestSuite(unittest.TestCase):
    ...
    def prepExtSrv(self,
        ...
        # jagonzal: In the ALMA branch the host id is allways host w/o port
        ...
        if (refCfgObj.getDbInterface().upper().find("SQLITE") != -1):
            ...
            refCfgObj.storeVal("NgamsCfg.Db[1].Name", sqliteDb)
	...
	# jagonzal: Use the python interpreter aas provided by the shell
    ...
    def tearDown(self):
	    ...
	    # jagonzal: Remove files retrieved to the local directory
            execCmd("rm -rf %s/ngamsTest/*fits.Z" % ngamsGetSrcDir(), 0)

        # jagonzal: Clean up db as well
	if (not getNoCleanUp()):
            cleanUp(getRefCfg(),0,1,0)
    ...
    def checkFilesEq(self,
        ...
        Returns:    Void.
        """
	diff = cmpFiles(refFile, tmpFile, sort)
	# jagonzal: Re-generate reference files
	if ((diff) and (os.getenv("GENERATE_REF_FILES"))):
	    stat, out = commands.getstatusoutput("cp %s ./%s" % (tmpFile,os.path.basename(refFile))) 
        self.failUnless("" == diff,genErrMsg(msg, refFile, tmpFile))
    ...
    def _substQueryVal(self,
        ...
        if ((queryVal.find("<DateTime object") != -1) or
            checkIfIso8601(queryVal) or
            # jagonzal: Take into account the '' around the time stamp
            checkIfIso8601(queryVal.replace("'",""))):
            queryVal = "_DATETIME_"
	...
	# jagonzal: Adapt to ALMA Oracle-based plugins
        resTag1 = "Leaving _fetchAll() with results:"
	resTag2 = "Executing query:"
	# jagonzal: We cannot assure anything about the date...
	excludeTag = "completion_date"
		...
		sqlRes = line.split("|")[1].split("|")[0]
                # jagonzal: I think it is better to get the query
                # plan only after the response has beign generated
		queryPlan += "%s: %s\n" % (sqlQuery, sqlRes)
            else:
                # jagonzal: Adapt to ALMA Oracle-based plugins
                # sqlQuery = line.split(": ")[1].split(" [ngamsDb")[0]
		sqlQuery = line.split("|")[1].split("|")[0]
		sqlRes = ""
            # jagonzal: I think it is better to get the query
            # plan only after the response has beign generated
            # queryPlan += "%s: %s\n" % (sqlQuery, sqlRes)
