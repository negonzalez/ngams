...
#******************************************************************************
#
# "@(#) $Id: ngamsSrvUtils.py,v 1.10 2012/03/03 21:26:07 amanning Exp $"
#
# Who       When        What
# --------  ----------  -------------------------------------------------------
...
def handleOffline(srvObj,
    ...
    info(1, "Prepare NG/AMS for Offline State ...")
    ...
    if (srvObj.getCfg().getSimulation() == 0):
        info(1, "Invoking System Offline Plug-In: " + plugIn +\
             "(srvObj, reqPropsObj)")
        exec "import " +  plugIn
    ...
    info(1,"NG/AMS prepared for Offline State")
...
def checkStagingAreas(srvObj):
    ...
    for filename in stagingFileDic.keys():
        info(2, "removing staging file: " + str(filename))
        os.remove(filename);
