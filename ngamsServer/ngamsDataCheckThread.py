...
#******************************************************************************
#
# "@(#) $Id: ngamsDataCheckThread.py,v 1.16 2012/03/03 21:20:54 amanning Exp $"
#
# Who       When        What
# --------  ----------  -------------------------------------------------------
...
def _dumpFileInfo(srvObj,
    ...
    ###########################################################################
    info(3,"Getting information about all disks mounted in this system ...")
    ...
    for diskInfo in diskListRaw:
        info(3, "checking disk: " + str(diskInfo))
    ...
    for dbmFile in dbmFileList:
        info(3, "considering: " + str(db,File))
        _stopDataCheckThr(srvObj)
    ...
    for diskId in _getDiskDic().keys():
        ...
        info(3, "creating temporary queue BSDB: " + str(tmpQueueDbmFile))
        queueDbm = ngamsDbm.ngamsDbm(tmpQueueDbmFile, 0, 1)

        # Get the theoretical number of files.
        noOfFilesList = []
        accuNoOfFiles = 0
        # TBD - why is this done 3 times? Some kind of weird voting system?
        for n in range(3):
        ...
        info(3, "expected number of files on disk: " + str(diskId) + " is " + str(expNoOfFiles))
        
        info(3, "fetching all files on disk from ngas_files, caching in the tmp queue BSDB")
        ...
        queueDbm.sync()
        del cursorObj
        info(3, "cached ngas_files in tmp queue BSDB")
        ...
        del queueDbm
        info(3, "rename: " + str(tmpQueueDbmFile) + " -> " + str(queueDbmFile))
        ...
        errorDbmFile = "%s/%s_ERRORS_%s.bsddb" %\
                       (cacheDir, NGAMS_DATA_CHECK_THR, diskId)
        info(3, "creating ERROR BSDB: " + str(errorDbmFile))
        errorDbm = ngamsDbm.ngamsDbm(errorDbmFile, 0, 1)
        _getDbmObjDic()[diskId] = (queueDbm, errorDbm)
    ...
    ###########################################################################
    info(3, "Create DBM with references to all files on the storage disks ...")
    fileRefDbm = "%s/%s_FILES_%s.bsddb" %\
                 (cacheDir, NGAMS_DATA_CHECK_THR, getHostId())
    info(3, "removing OLD GLOB DB: " + str(fileRefDbm))
    rmFile(fileRefDbm)
    info(3, "recreating GLOB DB: " + str(fileRefDbm))
    _setFileRefDbm(ngamsDbm.ngamsDbm(fileRefDbm, 1, 1))
    tmpGlobFile = tmpFilePat + "_FILE_GLOB.glob"
    for diskId in _getDiskDic().keys():
        info(3, "globbing " + str(diskId))
        _stopDataCheckThr(srvObj)
        ...
        while (1):
            ...
            tmpCmd = "find " + searchPath + " -maxdepth 1 -not -wholename \"*lost+found*\" > " + tmpGlobFile
            info(3, "globbing: " + str(tmpCmd))
            stat, out = commands.getstatusoutput(tmpCmd)
            if (stat != 0): break
            fo = open(tmpGlobFile)
            info(3, "adding files found to GLOB DB")
            while (1):
                _stopDataCheckThr(srvObj)
        ...
        _stopDataCheckThr(srvObj)
        rmFile(tmpGlobFile)
        info(3, "finished writing files to GLOB DB")
    ...
    for diskId in _getDiskIdList():
        ...
        for fileKey,dbVal in queueDbm.iteritems():
            # jagonzal: We need to reformat the values and skip administrative elements e.g. __length #################
            if str(fileKey).startswith("__"): continue
...
def _crossCheckNonRegFiles(srvObj):
    ...
    try:
        ...
        for filename,dbVal in _getFileRefDbm().iteritems():
            # jagonzal: We need to reformat the values and skip administrative elements e.g. __length #################
            if str(filename).startswith("__"): continue
            fileInfo = cPickle.loads(dbVal)
            #############################################################################################
            diskId = fileInfo[0]
            if (not _getDiskDic().has_key(diskId)):
...
def dataCheckThread(srvObj,
    ...
    while (dataCheckThreadLoop):
        ...
        # jagonzal: Stop Data Check thread if a mirroring process is running
        while (srvObj.getMirroringRunning()):
            time.sleep(60)
            continue

        # IEncapsulate this whole block to avoid that the thread dies in
        ...
        try:
            ...
            info(3, "removing: %s/%s/%s*" % (srvObj.getCfg().getRootDirectory(),
