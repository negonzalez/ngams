...
#******************************************************************************
#
# "@(#) $Id: ngamsCacheControlThread.py,v 1.13 2012/03/03 21:19:59 amanning Exp $"
#
# Who       When        What
# --------  ----------  -------------------------------------------------------
...
def createCacheDbms(srvObj):
    ...
    info(3, 'using cache db: ' + str(cacheContDbmName))
    ...
    info(3, "recreating new files BSDB: " + newFilesDbmName)
    ...
    cacheCtrlPiDbmName = "%s/%s_%s" % (cacheDir, NGAMS_CACHE_CTRL_PI_DBM,
                                       getHostId())
    info(3, "recreating checking BSDB: " + cacheCtrlPiDbmName)
    ...
    cacheCtrlPiDelDbmName = "%s/%s_%s" % (cacheDir,
                                          ...
                                          getHostId())
    info(3, "recreating deletion BSDB: " + cacheCtrlPiDelDbmName)
    ...
    cacheCtrlPiFilesDbmName = "%s/%s_%s" % (cacheDir,
                                            ...
                                            getHostId())
    info(3, "recreating remainInCache BSDB: " + cacheCtrlPiFilesDbmName)
...
def initCacheArchive(srvObj):
    ...
    # Update the NGAS Cache Tables (local and remote)
    updateNgasCacheTable(srvObj)
...
def updateNgasCacheTable(srvObj):
    """
    Update the NGAS Cache Tables (local and remote)
    ...
    info(3, "updating ngas cache table")
...
def checkCacheContents(srvObj):
    ...
    info(3, "starting to check cache contents")
    ...
    if (srvObj.getCfg().getVal("Caching[1].MaxTime")):
        info(3, "Applying criteria: Expired files ...")
        ...
        info(3, "creating BDB " + str(delFilesDbm))
        ...
        del delFilesDbm
        delFilesDbm = None
        info(3, "Applied criteria: Expired files")
          
    # 2. Remove files if there more files (in volume) in the cache than the
    #    specified threshold.
    info(3, "checking the cache size to see if it's too big")
    if (srvObj.getCfg().getVal("Caching[1].MaxCacheSize")):
        info(3, "Applying criteria: Maximum cache size ...")
        maxCacheSize = int(srvObj.getCfg().getVal("Caching[1].MaxCacheSize"))
        info(3, "the maximum cache size (from the configuration) is " + str(maxCacheSize))
        ...
        info(3, "the actual cache size is" + str(cacheSum))
        if (not cacheSum):
            cacheSum = 0
        else:
        ...
        if (cacheSum > maxCacheSize):
            ...
            del delFilesDbm
            delFilesDbm = None
        info(3, "Applied criteria: Maximum cache size")

    # 3. Remove files if there are more files in the cache than the
    #    specified threshold.
    info(3, "checking the number of files in the cache to see if it's too big")
    if (srvObj.getCfg().getVal("Caching[1].MaxFiles")):
        info(3, "Applying criteria: Maximum number of files ...")
        maxFiles = int(srvObj.getCfg().getVal("Caching[1].MaxFiles"))
        info(3, "maximumg number of files allowed (from configuration): " + str(maxFiles))
        sqlQuery = "SELECT count(*) FROM ngas_cache"
        numberOfFiles = queryCacheDbms(srvObj, sqlQuery)[0][0]
        info(3, "actual number of files: " + str(numberOfFiles))
        ...
        if (numberOfFiles > maxFiles):
            ...
            del delFilesDbm
            delFilesDbm = None
        info(3, "Applied criteria: Maximum number of files")
    ...
    if (0 and srvObj.getCfg().getVal("Caching[1].MinCacheSpace")):
        info(3, "Applying criteria: Minimum available cache space ...")
        ...
        if (spaceAvailMb < minCacheSpace):
            ...
            del delFilesDbm
            delFilesDbm = None
        info(3, "Applied criteria: Minimum available cache space")

    # 5. Invoke the Cache Control Plug-In (if specified) on the files.
    info(3, "applying the specified cache control plugin")
    if (srvObj.getCfg().getVal("Caching[1].CacheControlPlugIn")):
        info(3, "Applying criteria: Cache Control Plug-In ...")
        ...
        info(3, "waiting for all entries in Cache Control Plugin to be handled")
        while (True):
            checkStopCacheControlThread(srvObj)
            count = _getCountCacheCtrlPlugInDbm(srvObj)
            info(3, "current count: " + str(count))
            if (count == 2): break
            time.sleep(0.250)

        # Mark the files to be deleted for deletion.
        info(3, "marking files for deletion")
        while (True):
        ...
        info(3, "marking files as checked")
        while (True):
        ...
        info(3, "Applied criteria: Cache Control Plug-In")
    info(3, "finished checking cache contents")
...
def cacheControlThread(srvObj,
    ...
    while (True):
        ...
        try:
            checkStopCacheControlThread(srvObj)
            info(3, "Cache Control Thread starting next iteration ...")
            ...
            info(3, "Cache Control Thread executed - suspending for " +\
                 str(suspTime) + "s ...")
