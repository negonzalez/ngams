...
#******************************************************************************
#
# "@(#) $Id: ngamsFileUtils.py,v 1.15 2011/11/07 19:25:46 amanning Exp $"
#
# Who       When        What
# --------  ----------  -------------------------------------------------------
...
from ngams import *
import ngamsDbm, ngamsDbCore, ngamsDb, ngamsLib, ngamsFileInfo
...
def _locateArchiveFile(srvObj,
    ...
    if ((not fileDbm.getCount()) and (srvObj.getPartnerSites() == [])):
        tmpFileRef = fileId
    ...
    # If candidate files are available, from the local RDBMS, look for the
    # best suitable file.
    if (noOfFiles):
        ...
        for fileListInfo in fileLists:
            ...
            for fileInfo in fileList:
                ...
                if (not candFileDic.has_key(fileVer)):
                    candFileDic[fileVer] = []
                candFileDic[fileVer].append([location, fileInfo[0],
                                             fileInfo[1]])
        ...
        sqlDiskInfo = srvObj.getDb().\
                      getDiskInfoFromDiskIdList(diskIdDic.keys())
        diskInfoDic = {}
        ...
        # in the fileList is found, the information is returned as the file
        # wanted.
        ...
        for fileVer in fileVerList:
            ...
            for fileInfo in candFileDic[fileVer]:
                ...
                if (location == NGAMS_HOST_LOCAL):
                    # Check first if the local system supports retrieve
                    # requests (if relevant).
                    if (reqPropsObj):
                    ...
                    filename =\
                             os.path.normpath(diskInfoObj.getMountPoint() +\
                                              "/" +\
                                              fileInfoObj.getFilename())
                    info(3,"Checking if local file with name: " + filename +\
                         " is available ...")
                    if (not os.path.exists(filename)):
                        info(3,genLog("NGAMS_INFO_FILE_NOT_AVAIL",
                                      [fileId, host]))
                    else:
                    ...
                    info(3,"Checking if file with ID/Version: "+\
                         ...
                         " is available on host/port: " + host + "/" +\
                         str(port) + " ...")
                    ...
                    if (hostDic[host].getSrvSuspended() == 1):
                        info(3,"Server hosting requested file (" + host +\
                             "/" + str(port) + ") is suspended - " +\
                             "waking up server ...")
                        try:
                            ngamsSrvUtils.wakeUpHost(srvObj, host)
                            info(3,"Suspended server hosting requested " +\
                                 "file (" +\
                                 host + "/" + str(port) +\
                                 ") has been woken up")
                        except Exception, e:
                            warning("Error waking up server hosting " +\
                                    "selected file. Error: " + str(e))
                            continue
                    ...
                    if ((statusObj.getMessage().\
                        ...
                        info(3,genLog("NGAMS_INFO_FILE_AVAIL", [fileId, host]))
                        foundFile = 1
                        srcAddress = hostDic[host].getIpAddress()
                        break

    # If no file was found by now, and Partner Archives are defined in the
    # configuration, query these for the availability of the requested file.
    if ((not foundFile) and (srvObj.getPartnerSites() != [])):
        pars = [["file_access", fileId]]
        if (fileVersion != -1):
            pars.append(["file_version", fileVersion])
        for partnerSite in srvObj.getPartnerSites():
            authHdr = ngamsSrvUtils.genIntAuthHdr(srvObj)
            srcAddress = partnerSite.split(":")[1][2:]
            port = int(partnerSite.split(":")[-1])
            try:
                statusInfo = ngamsLib.httpGet(srcAddress, port,
                                              NGAMS_STATUS_CMD, 1, pars,
                                              authHdrVal=authHdr)
            except:
                # We ignore this error, and try the next site, if any.
                continue
            statusObj = ngamsStatus.ngamsStatus().\
                        unpackXmlDoc(statusInfo[3], 1)
            info(5,"Result of File Access Query: " +\
                 re.sub("\n", "", str(statusObj.genXml().\
                                      toprettyxml('  ', '\n'))))
            if ((statusObj.getMessage().find("NGAMS_INFO_FILE_AVAIL") == -1)):
                info(3,genLog("NGAMS_INFO_FILE_NOT_AVAIL",
                              [fileId, srcAddress]))
            else:
                info(3,genLog("NGAMS_INFO_FILE_AVAIL", [fileId, srcAddress]))
                foundFile = 1
                location = NGAMS_HOST_REMOTE
                host = srcAddress
                # Get the information about the remote file.
                pars = [["file_id", fileId]]
                if (fileVersion != -1):
                    pars.append(["file_version", fileVersion])
                statusInfo = ngamsLib.httpGet(srcAddress, port,
                ...
                statusObj = ngamsStatus.ngamsStatus().\
                            unpackXmlDoc(statusInfo[3], 1)           
                diskInfoObj = statusObj.getDiskStatusList()[0]
                fileInfoObj = diskInfoObj.getFileObjList()[0]
                break
    ...
    srcFileInfo = [location, host, srcAddress, port,
    ...
    format = "Located suitable file for request - File ID: %s. " +\
             ...
             "Mime-type: %s"
    info(2,format % (fileId, location, host, srcAddress, port,
...
def quickFileLocate(srvObj,
    ...
    if (res != [[]]):
        host_id = res[0][0][0]
        # If the host_id contains a port remove that
