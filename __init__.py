...
#******************************************************************************
#
# "@(#) $Id: __init__.py,v 1.31 2010/10/05 15:41:47 jagonzal Exp $"
#
# Who       When        What
# --------  ----------  -------------------------------------------------------
...
NGAMS_SRC_DIR = os.path.dirname(__file__)
...
NGAMS_ERR_DEF_FILE = os.path.normpath(ngamsGetSrcDir() + "/ngamsData/" +\
                                      "ngamsLogDef.xml")
...
def getDiskSpaceAvail(mountPoint,
    ...
    if (smart):
        if (_diskSpaceDic.has_key(mountPoint)):
            if ((startTime - _diskSpaceDic[mountPoint][0]) < 20):
    ...
    if (not diskSpace):
        ...
        exitCode, stdOut = commands.getstatusoutput("df -k %s " % mountPoint)
        if (exitCode):
            raise Exception, "Illegal path encountered: " +\
                  mountPoint + " - or path could not be created."
        diskSpace = eval(str(cleanList(stdOut.replace("\n", " ").\
                                       split(" "))[10]) + ".0")
