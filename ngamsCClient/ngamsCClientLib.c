...
/******************************************************************************
 *
 * "@(#) $Id: ngamsCClientLib.c,v 1.32 2013/02/18 08:18:52 amanning Exp $"
 *
 * who       when      what
 * --------  --------  --------------------------------------------------
...
/* static char *rcsId="@(#) $Id: ngamsCClientLib.c,v 1.32 2013/02/18 08:18:52 amanning Exp $"; */
...
void _setReplyTimeoutFromFloat(const float timeoutSecs) {
    // why? the default value is -1, which is "no timeout".
    // why is it then replaced with 2 mins?
    // for large files in ALMA this causes the client to terminate while the file is still being processed on the servers
    // frm the history it appears that achen decresed this value from 1000000 to 10. At some point it has been re-increased to
    // 2 minutes.
	if (timeoutSecs <= 0)
		_replyTimeout = 120;
	else
 ...
 */
int ngamsRecvHttpHdr(int* sockFd, ngamsHTTP_HDR httpHdr, ngamsHTTP_RESP* httpResp, ngamsHTTP_DATA* repDataRef, ngamsDATA_LEN* dataLen, float timeout) {
	...
	ngamsLogDebug("timeout: %f s", timeout);
	if ((retCode = _pollFd(*sockFd, timeout)) != ngamsSTAT_SUCCESS)
		goto errExit;
	...
	while ((bytesRead = ngamsReadLine(sockFd, recvLine, ngamsMAXLINE)) > 0) {
		...
		// treading very carefully (I know it's inefficient)
		// Previously we had SEGV errors in production from this point.
		// Some times we saw binary data in what should have been the header data.
		// I think it was due to the circular queue in ngamsArchiveClient being wrong, but I'm leaving this
		// very careful strcpy here until I gain confidence.
		if (hdrIdx >= 128) {
		    ngamsLogError("attempted to write past the end of the httpHdr array. Lost header: %s", recvLine);
		}
		else if (recvLine == NULL) {
		    ngamsLogError("received NULL as an httpHdr - ignoring");
		}
		else if (strlen(recvLine) >= 256) {
		    ngamsLogError("received line too big for httpHdr - ignoring. Lost header: %s", recvLine);
		}
		else {
	...
	if (bytesRead < 0) {
	    ngamsLogError("ngamsReadLine() returned %d: errno %d, errstr %s", bytesRead, errno, strerror(errno));
		retCode = ngamsERR_COM;
		goto errExit;
	}
...
int _ngamsHttpGet(const char* host, const int port, const char* userAgent, const char* path, const int receiveData, ngamsHTTP_DATA* repDataRef,
	...
	*dataLen = 0;
	...
	ngamsLogInfo(LEV1, "SENDING: %s", sendLine);
        n = strlen(sendLine);
	...
	if ((retCode = ngamsRecvHttpHdr(&sockFd, httpHdr, httpResp, repDataRef, dataLen, _getTimeOut())) != ngamsSTAT_SUCCESS) {
...
int _ngamsHttpPost(const char* host, const int port, const char* userAgent, const char* path, const char* mimeType, const char* contentDisp,
	...
	*repDataLen = 0;
	...
	// how long do we need as a timeout in order to receive the data header?
	// A scenario from ALMA - users ingest a 30GB file (really). The file transfers to the staging area. The client
	// waits for the return status header, but times out after 2 mins. The server requires about 10 minutes to copy the
	// file from the staging area to the final destination on disk.
	// calculate the timeout at a _very_ conservative 10MB/s
	const float ingestionRateBytesPerSecond = 10.0 * 1024.0 * 1024.0;
	// set a floor timeout of 2 mins, which was the old default value
	const float minTimeoutSecs = 120.0;
	float timeout = contLen / ingestionRateBytesPerSecond;
	if (timeout < minTimeoutSecs) timeout = minTimeoutSecs;
	ngamsLogDebug("calculated timeout %f s", timeout);
	...
	if (contLen) {
		if (*srcFilename != '\0') {
			...
			while ((bytesRead = read(fileFd, inBuf, 10240)) > 0) {
                ...
                int bytesWritten = write(sockFd, inBuf, bytesRead);
			...
			if (bytesRead < 0) {
				...
				retCode = ngamsERR_FILE;
				// workaround
				// strange bug in production:
				//   once in Xthousand times we saw that the full file was transferred to NGAS. However, this block was entered
				//   with bytesRead = -1, and the error was EAGAIN. Which doesn't make sense to me, the file is opened in blocking mode.
				//   The result was that ngamsArchiveClient thought it had failed to send the file and re-transmitted to NGAS, but NGAS 
				//   thought it had the full file, ingested it, and failed when attempting to send the status back to the client. The
				//   end result was a duplicate file in NGAS. 
				if (totalBytesWritten < contLen) {
				    ngamsLogInfo(LEV1, "wrote %d out of %d bytes. Error in transmission", totalBytesWritten, contLen);
				    // NGAS will not ingest the file because it has received less bytes than expected.
				    goto errExit;
				}
				else {
				    // in this case NGAS will have ingested the file. So we should just ignore the error and continue as normal.
				    ngamsLogInfo(LEV1, "despite the strange error, all bytes were sent. Proceeding normally.");
				    goto unsureExit;
				}
				// end of workaround
			}
	...
	/* Receive the reply for the request */
	if ((retCode = ngamsRecvHttpHdr(&sockFd, httpHdr, httpResp, repDataRef, repDataLen, timeout)) != ngamsSTAT_SUCCESS)
		goto unsureExit;
	...
	unsureExit:
	    // ngamsLogWarning("All data was sent, but response was not received. Did Ngas get the file? Not sure.");
...
int ngamsHttpPost(const char* host, const int port, const char* userAgent, const char* path, const char* mimeType, const char* contentDisp,
	...
	if (!foundList) {
		...
		while (tryCount < maxTries) {
			...
			if (stat == ngamsSTAT_SUCCESS)
				break;
			if (stat == ngamsERR_UNKNOWN_STAT) {
			    // give the server a chance to recover
			    ngamsSleep(5);
			    // there's not much we can do. We can assume that the file was transferred fine, but that's dangerous. It's best
			    // to re-transmit.
			    ngamsLogWarning("File %s was sent to NGAS, but no reply received. I'm not sure if it was ingested. Erring on the side of caution and re-transmitting. This may result in duplicate files in NGAS.", srcFilename);
			}
			tryCount += 1;
		}
		if (tryCount == maxTries) {
...
int ngamsHttpPostClose(int sockFd, ngamsHTTP_DATA* repDataRef, ngamsDATA_LEN* repDataLen, ngamsHTTP_RESP* httpResp, ngamsHTTP_HDR httpHdr) {
	...
	*repDataLen = 0;
	...
	/* Receive the reply for the request */
	if ((retCode = ngamsRecvHttpHdr(&sockFd, httpHdr, httpResp, repDataRef, repDataLen, _getTimeOut())) != ngamsSTAT_SUCCESS)
		goto errExit;
