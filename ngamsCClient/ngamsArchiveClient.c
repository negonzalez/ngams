...
/******************************************************************************
 *
 * "@(#) $Id: ngamsArchiveClient.c,v 1.21 2013/02/18 08:18:52 amanning Exp $"
 *
 * who       when      what
 * --------  --------  --------------------------------------------------
...
/* static char *rcsId="@(#) $Id: ngamsArchiveClient.c,v 1.21 2013/02/18 08:18:52 amanning Exp $"; */
		     ...
		     const char*   key)
{
    ...
    ngamsMED_BUF  tmpKey;
    if (safeStrCp(tmpKey, key, sizeof(ngamsMED_BUF)) == ngamsSTAT_FAILURE) {
        ngamsLogInfo(LEV5, "failed to safe str cpy: %s", key);
	return ngamsSTAT_FAILURE;
}
    dbmKey.data = tmpKey;
    dbmKey.size = strlen(key);
	...
	return ngamsSTAT_SUCCESS;
        ...
        ngamsLogInfo(LEV5, "dbmsDel failed:");
	return ngamsSTAT_FAILURE;
}
}
		      ...
		      const ngamsMED_BUF             sourceFile)
{
    ...
    // ngamsLogInfo(LEV3, "ADD rd idx %d, wr idx %d", regP->archiveQueueRdIdx, regP->archiveQueueWrIdx);
    // don't exceed the capacity of the queue. Previous version allowed overwrites. While not wrong in theory,
    // we did see various problems occurring such as hangs, duplicate files being ingested.
    if ((regP->archiveQueueWrIdx + 1) % ngamsMAX_FILES_IN_Q == regP->archiveQueueRdIdx) {
        ngamsLogInfo(LEV3, "ADD: Q is full - not adding any new files");
        pthread_mutex_unlock(&(regP->archiveQueueMutex));
        return ngamsSTAT_SUCCESS;
    }

    /* Check if this file is already being processed */
    if (hasKeyDbm(regP->queueDbmHandle, sourceFile) || hasKeyDbm(regP->procDbmHandle, sourceFile))
	{
        /* OK, assume file is already being processed - do nothing */
	    pthread_mutex_unlock(&(regP->archiveQueueMutex));
        ngamsLogInfo(LEV3, "ADD: %s is already being processed", sourceFile);
	    return ngamsSTAT_SUCCESS;
	}

    /* Create a new entry in the queue + DBM */
    // previously this was: newWrIdx = ((regP->archiveQueueWrIdx + 1) % ngamsMAX_FILES_IN_Q);
    // now I've changed the indexing to a classic circular buffer _with no overwrite_
    int newWrIdx = regP->archiveQueueWrIdx;
    ngamsLogInfo(LEV3, "ADD: writing to index %d - %s", newWrIdx, sourceFile);
    ...
    regP->archiveQueueWrIdx = (newWrIdx + 1) % ngamsMAX_FILES_IN_Q;
    (regP->archiveQueueCount)++;
    ngamsLogInfo(LEV3, "ADD: new count: %d", regP->archiveQueueCount);
			    ...
			    ngamsMED_BUF                   sourceFile)
{
    pthread_mutex_lock(&(regP->archiveQueueMutex));
    *sourceFile = '\0';
    // ngamsLogInfo(LEV3, "GET: rd idx: %d, wr idx %d", regP->archiveQueueRdIdx, regP->archiveQueueWrIdx);

    // previously: int newRdIdx = ((regP->archiveQueueRdIdx + 1) % ngamsMAX_FILES_IN_Q);
    // now it's a classic circular queue _with no overwrite_ of existing elements in the queue
    int newRdIdx = regP->archiveQueueRdIdx;
    if (newRdIdx == regP->archiveQueueWrIdx) {
        ngamsLogInfo(LEV4, "GET: nothing to read");
        pthread_mutex_unlock(&(regP->archiveQueueMutex));
        return ngamsSTAT_SUCCESS;
    }
    // this should never happen, but left to check, as it was a problem in the old implementation
    if (*(regP->archiveQueue[newRdIdx]) == '\0')
	{
        ngamsLogInfo(LEV3, "GET: There is no file at position: %d", newRdIdx);
     ...
     */
    ngamsLogInfo(LEV3, "GET: filename: %s", regP->archiveQueue[newRdIdx]);
    if (safeStrCp(sourceFile, regP->archiveQueue[newRdIdx],
		  sizeof(ngamsMED_BUF)) == ngamsSTAT_FAILURE)
	{
        ngamsLogInfo(LEV3, "GET: fail1");
	    pthread_mutex_unlock(&(regP->archiveQueueMutex));
	    return ngamsSTAT_FAILURE;
	}
    ...
    *(regP->archiveQueue[newRdIdx]) = '\0';
    regP->archiveQueueRdIdx = (newRdIdx + 1) % ngamsMAX_FILES_IN_Q;
...
void* _archiveQMonThread(void*  ptr)
{
    ...
    while (1)
	{
        ngamsLogInfo(LEV5, "starting next monitor thread iteration");
         ...
         */
        ngamsLogInfo(LEV5, "archive queue count: %d", regP->archiveQueueCount);
        ngamsLogInfo(LEV5, "num streams: %d", regP->streams);
        ...
        ngamsLogInfo(LEV5, "sleeping for %d", regP->archiveQueuePollTime);
...
void* _archiveThread(void*  ptr)
{
    ...
    while (1)
	{
       ngamsLogInfo(LEV4, "fetching from Q"); 
...
ngamsSTAT ngamsServe(ngamsARCHIVE_CLIENT_REGISTRY*  regP)
{
    ...
    // we had problems in production with the ocassional SIGPIPE error. We ignore it.
...
ngamsSTAT ngamsCheckArchiveQueue(ngamsARCHIVE_CLIENT_REGISTRY*  regP)
{
    ...
    while ((dirEnt = readdir(dirPtr)) != NULL)
	{
        if (dirEnt->d_name[0] != '.')
        {
            ...
            {
                sprintf(queueFilename, "%s/%s", ngamsGetQueueDir(regP->rootDir), dirEnt->d_name);
            ...
            {
                // don't log this as an error - we will quickly fill up all the disk space with the logfile
                ngamsLogInfo(LEV3, "%s is not a normal file. ngamsArchiveClient doesn't do nested directories.", dirEnt->d_name);
	            ngamsMED_BUF removeName;
	            sprintf(removeName, "%s/%s", ngamsGetQueueDir(regP->rootDir), dirEnt->d_name);
			    ngamsMED_BUF isoTime;
                ngamsGenIsoTime(3, isoTime);
                ngamsMED_BUF badName;
	            sprintf(badName, "%s/%s___%s", ngamsGetBadDir(regP->rootDir), isoTime, dirEnt->d_name);
                int retCode = rename(removeName, badName);
                if (retCode == 0) {
                    ngamsLogWarning("%s was moved to bad-files", dirEnt->d_name);
                }
                else {
                    ngamsLogInfo(LEV3, "Couldn't move the file to the bad-files directory (%s) - please do it manually.", strerror(errno));
                }
	        }
        }
...
ngamsSTAT ngamsCleanUpArchivedFiles(ngamsARCHIVE_CLIENT_REGISTRY*  regP)
{
    ...
    while ((dirEnt = readdir(dirPtr)) != NULL)
    {
         ...
         */
        if ((dirEnt->d_name[0] != '.') && (strstr(dirEnt->d_name, ngamsARCH_CLI_STAT_EXT) == NULL))
        {
